...
_PREFIX_TO_PROVIDER = {
    p["model_prefix"]: p["id"] for p in MODEL_PROVIDERS if p.get("model_prefix")
}


def _detect_provider(model_string: str) -> str | None:
    """Match a model string to its provider ID by prefix."""
...
class EnvConfigManager:
    ...
    def load_models(self) -> list[ModelConfig]:
        ...
        for model_string in (m.strip() for m in inspect_model.split(",") if m.strip()):
            provider_id = _detect_provider(model_string)
    ...
    def get_var(self, name: str) -> str:
        """Get a single env var from .env file. Returns empty string if missing."""
        return self._read_env().get(name, "")

    def set_var(self, name: str, value: str) -> None:
        """Set or update an env var."""
        if not _VALID_ENV_KEY.match(name):
