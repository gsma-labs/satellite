...
from satellite.examples.eval_data import MODEL_PROVIDERS
from satellite.services.config.env_config_manager import normalize_model_path
...
from satellite.services.submit.parquet_builder import build_model_card_parquet
...
def _build_pr_body(preview: SubmitPreview) -> str:
    ...
    lines = [
        ...
        f"**Files:** 1 model card (parquet, from {len(preview.log_files)} eval logs)",
        "",
        "### Benchmark Scores",
        "",
        "| Benchmark | Score |",
    ...
    lines.extend(["", "---", "*Submitted via Satellite TUI*"])
    return "\n".join(lines)
...
def _do_submit(client: GitHubClient, preview: SubmitPreview) -> SubmitResult:
    ...
    remote_path, parquet_bytes = build_model_card_parquet(preview)
    client.upload_files(target_repo, branch_name, [(remote_path, parquet_bytes)])

    title = f"Add {preview.provider}/{preview.model_dir_name} evaluation results"
